...
public extension UIFont {
    ...
    static func oog(_ type: OOGFontType,_ ipadSize:CGFloat, _ iphoneSize:CGFloat) -> UIFont{
        return UIFont.init(name: type.name, size: cIpad(ipadSize, ipadSize)) ?? UIFont.systemFont(ofSize: cIpad(ipadSize, ipadSize))
    }

    //传入真实的尺寸不需要转换
    static func oogR(_ type: OOGFontType,_ ipadSize:CGFloat, _ iphoneSize:CGFloat) -> UIFont{
        return UIFont.init(name: type.name, size: cIpad(ipadSize, ipadSize)) ?? UIFont.systemFont(ofSize: rIpad(ipadSize, ipadSize))
    }
